      ...
      fn h ->
        ...
        |> body(fn h ->
          core_func =
            case s.__struct__ do
              BlackBox -> & &1
              Schematic -> &print_html_logics(&1, s, as: hier)
            end
        ...
        |> ul(fn h ->
          Enum.reduce(subs, h, fn i_name, h ->
            ...
            case inst.__struct__ do
              ChoiceLogic ->
                print_html_choice_logic(h, inst, as: i_name)

              ConditionLogic ->
                print_html_condition_logic(h, inst, as: i_name)

              CaseLogic ->
                print_html_case_logic(h, inst, as: i_name)

              SeqLogic ->
                print_html_seq_logic(h, inst, as: i_name)

              FiniteStateMachine ->
                print_html_fsm_logic(h, inst, as: i_name)

              Assertion ->
                print_html_assertion(h, inst, as: i_name)

              Sink ->
                print_html_sink(h, inst, as: i_name)
