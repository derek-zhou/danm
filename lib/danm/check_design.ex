defmodule Danm.CheckDesign do
  ...
  defp module_to_key(s) do
    case s.__struct__ do
      ...
      # anything else should always has a unique key
      _ -> s
    end
  end
  ...
  defp begin_check(state, key, s) do
    %{state | dict: Map.put(state.dict, key, :ongoing), stack: [s | state.stack]}
  end

  defp end_check(state, key) do
    %{state | dict: Map.put(state.dict, key, :done), stack: tl(state.stack)}
  end
  ...
  defp check_design(state, s) do
    ...
    case Map.get(state.dict, key) do
      ...
      :ongoing ->
        error(state, "Infinite recursive design")
  ...
  defp calculate_cache_data(state) do
    %{
      state
      | cache:
          Map.put(state.cache, :map, state |> current_design() |> Schematic.pin_to_wire_map())
    }
  end
  ...
  defp check_fsm_logic(state) do
    ...
    warning(state, "output looped back to condition clause of a FSM", if: ComboLogic.loop_back?(s))
  end
end
